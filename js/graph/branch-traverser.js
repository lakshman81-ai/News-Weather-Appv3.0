...
export const detectStartNodes = (groups) => {
  ...
  for (const [refno, group] of groups) {
    ...
    const hasStart = group.rows.some(r => String(r.rigid ?? r.Rigid ?? '').toUpperCase() === 'START');
 ...
 * Main run: the neighbour connected via EP1 or EP2.
 * Branch: the neighbour connected via BP (Point=3).
...
const _classifyTeeNeighbours = (teeRefno, topology, groups) => {
  const teeGroup = groups.get(teeRefno);
  const pts = teeGroup?.pts ?? {};
  const neighbours = topology.adj.get(teeRefno) ?? [];

  const branchCoord = pts['3']
    ? { E: pts['3'].E, N: pts['3'].N, U: pts['3'].U }
    : null;

  if (!branchCoord) return { mainContinuation: neighbours, branchStarts: [] };

  const mainContinuation = [];
  const branchStarts = [];

  for (const nRefno of neighbours) {
    const nGroup = groups.get(nRefno);
    if (!nGroup) continue;
    const nPts = nGroup.pts ?? {};
    // Check if neighbour's EP1 matches branch point
    const ep1 = nPts['1'] ?? nPts['0'] ?? null;
    if (!ep1) { mainContinuation.push(nRefno); continue; }

    const dE = Math.abs(ep1.E - branchCoord.E);
    const dN = Math.abs(ep1.N - branchCoord.N);
    const dU = Math.abs(ep1.U - branchCoord.U);
    const dist = Math.sqrt(dE*dE + dN*dN + dU*dU);

    if (dist < 1.0) {
      branchStarts.push(nRefno);
    } else {
      mainContinuation.push(nRefno);
    }
  }
