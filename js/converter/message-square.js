...
export const buildMsgSquare = (pts, pcfType, config) => {
  ...
  let text = interpolate(template, vars).replace(/,\s*,/g, ',').replace(/,\s*$/, '').trim();

  // Inject RefNo if available (passed via config or argument - plan to add optional argument)
  if (config.refno) {
    text += `  RefNo:${config.refno}`;
  }
...
export const buildBendMsgSquare = (pts, angleStr, config) => {
  ...
  let text = interpolate(template, vars).trim();
  if (config.refno) {
    text += `  RefNo:${config.refno}`;
  }
