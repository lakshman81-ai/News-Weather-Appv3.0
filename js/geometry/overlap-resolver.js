 ...
 */
const _innerComponentSpans = (groups, pipeRefno, ep1, ep2, pipeBore, tol, boreTol, relaxedUpper) => {
  const spans = [];
 ...
 */
const _tracePathChain = (groups, pipeRefno, seedCoord, pipeBore, tol, boreTol) => {
  const chain = [];
  ...
  for (const [, g] of groups) {
    ...
    // At least one endpoint must be bore-compatible
 ...
 */
const _dominantAxisScan = (groups, pipeRefno, ep1, ep2, pipeBore, tol, boreTol, inferredEp2) => {
...
const _detectSameTypeOverlaps = (groups, tol, boreTol, commonLogic) => {
  const anomalies = [];
  const groupList = [...groups.values()].filter(g => !g.skip && g.pts);
      ...
      if (overlapLen > tol) {
          ...
          if (!anomalies.find(x => x.id === id)) {
            anomalies.push({
              ...
              detail: `${isExcessive ? `[EXCESSIVE > ${maxOverlap}mm] ` : ''}Overlap Length=${overlapLen.toFixed(1)}mm. Review source CSV.`,
...
const _fillSequenceGaps = (groups, tol, minPipeLen, boreTol, commonLogic, ignoreSupports) => {
  ...
  for (const [, g] of groups) {
    ...
    // In Pass 2 (ignoreSupports=true), exclude Supports from the gap-fill candidates entirely.
    // They should not be used as endpoints for bridging gaps.
    if (ignoreSupports && (g.pcfType === 'SUPPORT' || g.pcfType === 'ANCI')) continue;
  ...
  const getExit = (g) => {
    if (g.pcfType === 'SUPPORT') return g.pts?.['0'] ?? null;
  ...
  for (let i = 0; i < fittings.length - 1; i++) {
    ...
    // Check Max Diagonal Gap for indeterminate components
    // If a component has indeterminate direction (Support/Olet), we allow connection if:
    // 1. Strictly axis-aligned (Standard)
    // 2. OR Diagonal gap is small enough (< maxDiagonalGap) (Short Diagonal)
    // 3. OR Rules are disabled
    const maxDiagonal = rulesEnabled ? (commonLogic?.maxDiagonalGap ?? 6000) : Infinity;
    const isAxisAligned = (v) => Math.abs(v.E) > 0.9 || Math.abs(v.N) > 0.9 || Math.abs(v.U) > 0.9;
    const isShortDiagonal = (len) => len <= maxDiagonal;

    let alignedA = false;
    if (isNonZero(aVec)) {
      alignedA = _dot(gapVec, aVec) > 0.9;
    } else {
      // Indeterminate direction (SUPPORT/OLET)
      alignedA = isAxisAligned(gapVec) || isShortDiagonal(gap);
    }
    ...
    if (isNonZero(bVec)) {
      ...
      alignedB = isAxisAligned(gapVec) || isShortDiagonal(gap);
    }
...
export const resolveOverlaps = (groups, config) => {
  ...
  // Only sort if rules are enabled, otherwise keep CSV order?
  // actually sorting is good practice regardless, but let's stick to the toggle logic.
  ...
  for (const [refno, group] of sortedGroups) {
    ...
    if (group.pcfType !== 'PIPE' || group.skip || !ep1) {
      result.set(refno, group);
      continue;
    }
    ...
    const spans = _innerComponentSpans(
      groups, refno, ep1, ep2, pipeBore, tol, boreTol, inferredEp2
    );
    ...
    if (spans.length === 0) {
      ...
      const chain = _tracePathChain(groups, refno, ep1, pipeBore, tol, boreTol);
  ...
  if (settings.gapFillEnabled !== false) {
    ...
    _fillSequenceGaps(result, tol, minPipeLen, boreTol, commonLogic, ignoreSupports);
  }
