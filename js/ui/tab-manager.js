...
const TABS = ["input", "mapping", "table-view", "validate", "sequence", "preview", "output", "viewer", "config", "master-data"];
...
export function initTabManager() {
  TABS.forEach(id => {
    ...
    if (!btn) { console.warn(`${LOG_PREFIX} Tab button not found: tab-${id}`); return; }
    if (!panel) { console.warn(`${LOG_PREFIX} Tab panel not found: panel-${id}`); return; }
