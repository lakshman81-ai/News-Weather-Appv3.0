...
async function loadDefaultReferenceData() {
    // Try to fetch from relative path
    try {
        const response = await fetch("Docs/wtValveweights.xlsx");
        if (response.ok) {
            const blob = await response.blob();
            const file = new File([blob], "wtValveweights.xlsx", { type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" });
            await weightService.loadReferenceData(file);
        } else {
            console.warn("Could not auto-load reference data (404).");
            // Could provide a fallback or ask user to upload
            document.getElementById("weight-ref-status").innerText = "Not Found (Upload manually?)";
        }
    } catch (e) {
        console.warn("Auto-load reference data failed", e);
    }
}
